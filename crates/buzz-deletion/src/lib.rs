...
{
    let versions = if storage_version >= 5 {
        chunk
            .keys
            .iter()
            .map(|entry| {
...
async fn delete_validate_and_checkpoint_manifest_chunk_with<F, Fut, M, MFut>(
    chunk: buzz_db::deletion::ManifestKeyChunk,
    storage_version: i32,
    delete: F,
    mark_deleted: M,
...
where
    F: FnOnce(Vec<ObjectVersionRef>) -> Fut,
    Fut: Future<Output = Result<BulkDeleteOutcome>>,
    M: FnOnce(i64, serde_json::Value) -> MFut,
    MFut: Future<Output = Result<()>>,
{
    let outcome = delete_manifest_chunk_with(&chunk, storage_version, delete).await?;
    let detail = manifest_chunk_deleted_checkpoint_detail(&chunk, &outcome)?;
    mark_deleted(chunk.chunk_no, detail).await?;
    Ok(outcome)
}
        ...
        DeletionStage::Drained => {
            ...
            while let Some(chunk) = services.store.next_pending_manifest_chunk(&token).await? {
                let outcome = run_guarded_external_step(
                    ...
                    heartbeat_lost,
                    || async {
                        let mark_token = token.clone();
                        delete_validate_and_checkpoint_manifest_chunk_with(
                            chunk,
                            storage.version,
                            |versions| async {
                            ...
                            |chunk_no, detail| async move {
                                services
                                    .store
                                    .mark_manifest_chunk_deleted(&mark_token, chunk_no, detail)
                                    .await?;
                                Ok(())
                            },
                        )
...
mod tests {
    ...
    async fn partial_delete_ack_fails_before_checkpoint_detail() {
        ...
        let mark_flag = Arc::clone(&mark_called);
        let checkpoint = delete_validate_and_checkpoint_manifest_chunk_with(
            chunk,
            5,
            |versions| async move {
            ...
            |_, _| async move {
                mark_flag.store(true, Ordering::Relaxed);
                Ok(())
            },
        )
        .await;
        ...
        assert!(
            !mark_called.load(Ordering::Relaxed),
            "partial acknowledgement must not reach mark_manifest_chunk_deleted"
        );
    }
